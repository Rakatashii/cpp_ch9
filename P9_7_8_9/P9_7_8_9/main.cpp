/*
 ...
 Exercise P9.8.
 The program in Section 9.6 asks the user to specify the record number. More likely than not, a user has no way of knowing the record number.
 Write a program that asks the user for the name of an employee, finds the record with that name, and displays the record. Then the program should give the following options to the user:
    • Change the salary of this record
    • View the next record
    • Find another employee • Quit
 
 Exercise P9.9.
 To find a particular employee in a database file, the program needs to search one record at a time.
 If the records are sorted, there is a faster way. Count the number of records in the file, by dividing the length of the file by the length of each record.
 Set a variable first to 1, last to nrecords. Compute mid = (first + last)/ 2.
 Read the record at mid. Maybe you are lucky, and you actually found the record you wanted.
 If so, print it and exit. Is its name before or after the name that you are searching?
 Adjust either last to mid - 1 or first to mid + 1 and repeat the search.
 This searching method is called a binary search, and it is much faster than a sequential search through all records. Implement this searching method.
 */
// These exercises could have been accomplished with far less code. Focus here was more about experimenting with different storage and retrieval techniques - parallel vectors, namespacing the helper functions, storing file records as Employee objects, odd use of Employee#operator[], etc.
...
int main(int argc, const char * argv[]) {
    ...
    string db_original = "/Users/christianmeyer/cpp/ch9/P9_7_8_9/db_original";
    // This file is the output once the record are formatted for seekg
    string db_original_output = "/Users/christianmeyer/cpp/ch9/P9_7_8_9/db_original_output";
    ...
    string db_employees = "/Users/christianmeyer/cpp/ch9/P9_7_8_9/db_employees";
    // new output after Employee data members are changed.
    string db_employees_output = "/Users/christianmeyer/cpp/ch9/P9_7_8_9/db_employees_output";
    // this is the output for the helper function #sort_lines_by_field. While could have simply overwritted 'db_employees_output', it was helpful to be able to compare the files, and to leave each file operated on as they are in order to track all of the changes.
    string db_employees_sorted = "/Users/christianmeyer/cpp/ch9/P9_7_8_9/db_employees_sorted";
